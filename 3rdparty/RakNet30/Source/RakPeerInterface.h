...
public:
	...
	/// Sets the manager used to look up NetworkIDs for object member RPC.
	virtual void SetNetworkIDManager( NetworkIDManager *manager )=0;

	/// Returns the manager previously passed to SetNetworkIDManager, or 0.
	...
	/// Returns true if a particular system is connected to us.
	...
	/// Sends a datagram with TTL 1, so it opens the local router's NAT mapping without
	/// reaching the remote system.
	virtual void SendTTL1( const char* host, unsigned short remotePort, unsigned connectionSocketIndex )=0;
	...
	/// Allocates a Packet the caller can hand back through PushBackPacket.
	...
	/// Limits how many bits per second this system may send. 0 for unlimited.
