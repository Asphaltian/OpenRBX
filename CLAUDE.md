...
`webservice-statics.csv` matches by bare name, so a name under three characters stays out of it. Body's state counter is a file-scope `p`, and letting it through paired our `int` with somebody else's `double` and failed `reccmp-datacmp`. The two functions that read it hold at 62 percent on identical instruction streams; that is the cheaper price.
