...
That lines the constructors' eleven vftable stores up with the original's, at 0, 0x10, 0x14, 0x20, 0x24, 0x3c, 0x54, 0x6c, 0x84 and 0x9c. What is left is that the original's construction vftables are unnamed: naming the 860 `??_7?$FactoryProduct@` and `??_7?$DescribedCreatable@` publics takes the constructor from 31 to 62 percent, but `reccmp-vtable` then compares 307 tables and 281 fail, because a table named only by a data source has no `// VTABLE:` annotation proving its slots. Until that is separable, the constructors stay short.
