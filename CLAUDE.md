...
`getFaceCorners` is the one left, and what blocks it is a temporary count. The original's frame is 0x30, four `Vector3` slots that all six cases share, taken in ascending order by the first case and descending by the rest; ours is 0x120, four per case, which also costs the tail merge that shares the fourth `getCorner` call. MSVC 8 reuses a return temporary across sequential statements in a plain block, allocates a fresh one per statement inside a `switch` case, and never reuses across cases: one case gives 0x30, two give 0x60, six give 0x120, and braces, `goto`, a `default`, an inlined helper, four named locals and a comma expression all leave it there. Four live at once and reused across all six is what an inlined function whose body is one comma expression produces, `c0 = getCorner(i0), c1 = getCorner(i1), c2 = getCorner(i2), c3 = getCorner(i3);` under `__forceinline`, which reproduces the frame, the interleaved call and copy, and all 19 calls in a reduced case; the same shape in `Extents.cpp` does not inline. The compiler is not the difference: `S_COMPILE2` for `Extents.obj` records the same 14.00.50727 front and back end as ours, and compiling with that record's own command line changes nothing.
