...
Data past the end of `.data`'s raw bytes cannot go in any reccmp table. `jobs` and `dotVersion` both live there, so `reccmp-datacmp` has nothing to read and reports an error, and the loads from them stay unnamed: that is the whole of what `TerminateExtension` and `GetVersion` still diff on.
