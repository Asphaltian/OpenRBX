...
public:
	...
	virtual G3D::Rect2D getTextureSize(G3D::ReferenceCountedPointer<TextureProxyBase> _texture)
		const;

	virtual void line2d(const G3D::Vector2& p0, const G3D::Vector2& p1, const G3D::Color4& color)
		const;
	virtual void outlineRect2d(const G3D::Rect2D& rect, float thick, const G3D::Color4& color)
		const;
	...
	virtual void lineSegment(
		const G3D::LineSegment& lineSegment,
		const G3D::Color4& color,
		float scale
	);
	virtual void axes(
		const G3D::Color4& xColor,
		const G3D::Color4& yColor,
		const G3D::Color4& zColor,
		float scale
	);
